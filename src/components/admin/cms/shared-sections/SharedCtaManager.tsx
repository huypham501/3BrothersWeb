...
import { CmsSharedSection, sharedCtaSchema, SCHEMA_KEYS } from '@/lib/cms';
...
import {
  SharedCtaFields,
  SharedCtaFormValues,
  getSharedCtaDefaultValues,
} from '@/components/admin/cms/editors/SharedCtaEditor';

type FormValues = SharedCtaFormValues & { enabled: boolean };

interface SharedCtaManagerProps {
  section: CmsSharedSection<z.infer<typeof sharedCtaSchema>>;
  usageRoutes: string[];
  role: string;
  canPublish: boolean;
}

export function SharedCtaManager({
  section,
  usageRoutes,
  role,
  canPublish,
}: SharedCtaManagerProps) {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...getSharedCtaDefaultValues(section.content),
        ...
        <AdminAlert tone="warning">
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 8 }}>
            {usageRoutes.map((route) => (
              <AdminBadge key={`cta-${route}`}>{route}</AdminBadge>
            ))}
          </div>
        </AdminAlert>
